...
from config import (
    CF_R2_ACCESS_KEY_ID, CF_R2_SECRET_ACCESS_KEY, CF_R2_BUCKET_NAME,
...
class R2Uploader:
    ...
    def __init__(self):
        self.s3_client = boto3.client(
            's3',
            endpoint_url=CF_R2_ENDPOINT_URL,          # REQUIRED for R2
            aws_access_key_id=CF_R2_ACCESS_KEY_ID,
            aws_secret_access_key=CF_R2_SECRET_ACCESS_KEY,
            region_name='us-east-1',                   # dummy value, R2 ignores it
            config=Config(
                signature_version='s3v4',              # explicit sig version needed
    ...
    def upload_image_from_url(self, image_url: str, filename: str, s3_path: str = R2_IMAGES_PATH) -> Optional[str]:
        if not image_url:
        ...
        try:
            ...
            logger.info(f"Uploaded image to r2://{self.bucket_name}/{s3_key}")
            return s3_key
    ...
    def upload_local_file(self, local_path: str, s3_path: str, filename: str = None) -> Optional[str]:
        try:
            ...
            logger.info(f"Uploaded file to r2://{self.bucket_name}/{s3_key}")
            return s3_key
        except Exception as e:
    ...
    def get_r2_url(self, s3_key: str) -> str:
        return f"r2://{self.bucket_name}/{s3_key}"
