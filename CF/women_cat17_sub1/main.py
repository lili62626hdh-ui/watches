...
from config import TEMP_DIR, R2_EXCEL_PATH
...
# Hardcoded subcategory URLs for Watches
SUBCATEGORY_URLS = [
    "https://www.boutiqaat.com/ar-kw/women/watches/smart-watches-323469442/l/",
    "https://www.boutiqaat.com/ar-kw/women/watches/sports-watches-1921744616/l/",
    "https://www.boutiqaat.com/ar-kw/women/watches/fashion-watches-146678028/l/",
    "https://www.boutiqaat.com/ar-kw/women/watches/fine-watches-306107102/l/",
    "https://www.boutiqaat.com/ar-kw/women/watches/gift-sets-1410984929/l/",
    "https://www.boutiqaat.com/ar-kw/women/fashion-tech/smartwatch-accessories-1/l/",
    "https://www.boutiqaat.com/ar-kw/women/watches/kids-watches/l/",
]
...
class BoutiqaatDataPipeline:
    ...
    def run(self) -> bool:
        ...
        logger.info("=" * 80)
        logger.info("Starting Boutiqaat Data Pipeline (Async – Semaphore=3) [Cloudflare R2]")
        logger.info("=" * 80)
        try:
    ...
    def _process_category(self, scraper: BoutiqaatScraper, category: dict) -> bool:
        ...
        try:
            ...
            if not products:
                logger.info(f"Skipping {category_name} - no products available")
                return True  # Empty category is not a failure, just skip it
    ...
    def _upload_product_image(self, product: Dict, category_name: str, subcategory_name: str) -> str:
        try:
            ...
            r2_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/watches/images/"
                f"{safe_category}"
            )
    ...
    def _upload_excel_file(self, local_path: str, category_name: str) -> bool:
        try:
            ...
            r2_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/watches/excel-files"
            )
            ...
            if r2_key:
                logger.info(f"Excel file uploaded to R2: {r2_key}")
                return True
            else:
